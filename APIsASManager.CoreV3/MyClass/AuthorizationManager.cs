...
namespace APIsASManager.CoreV3.MyClass
{
    ...
    public class AuthorizationManager : IAuthorizationManager
    {
        ...
        {
            int vTotal = Convert.ToInt16(configuration.GetSection("MySettings").GetSection("Total").Value.ToString());
            for (int i = 0; i <= vTotal; i++)
            {
                var vTemp = configuration.GetSection("MySettings").GetSection("APIKey" + i).Value;
                string[] vSplit = vTemp.Split(';');
                users.Add(new User() { Name = vSplit[0], Pwd = vSplit[1], Role = vSplit[2] });
            }
            tokenTimeout = Convert.ToInt16(configuration.GetSection("MySettings").GetSection("TokenTimeout").Value);
        }
